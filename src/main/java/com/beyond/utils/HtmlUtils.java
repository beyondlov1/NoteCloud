...
public class HtmlUtils {
     ...
     * @return
     */
    public static String parseHtml2Text(String htmlText) {
        Html2Text html2Text = new Html2Text();
        html2Text.parse(htmlText);
        return html2Text.getText();
    }

    public static String addGithubMarkDownCss(String content) {
        return "<html><head><style>"+GITHUB_MARKDOWN_CSS+"</style></head><body class='markdown-body'>" +
                content +
                "</body></script></html>";
    }

    static class Html2Text extends HTMLEditorKit.ParserCallback {
        ...
        public void parse(String htmlText) {
            ...
            try {
                delegator.parse(new StringReader(htmlText), this, Boolean.TRUE);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
